...
export async function POST(request:Request){
 try{
  ...
  if(typeof body.text!=='string'||!body.text.trim()||body.text.trim().length>2000||typeof body.sessionId!=='string'||!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(body.sessionId))return Response.json({code:'simulatorInvalidMessage'},{status:400});
  const response=await auth.call('/api/admin/simulator',{method:'POST',body:JSON.stringify({text:body.text.trim(),sessionId:body.sessionId})});
