...
export function normalizeTenantSettings(value:unknown){const raw=object(value);return{
 ...
 time_zone:typeof raw.time_zone==='string'?raw.time_zone:'Asia/Jerusalem',supported_time_zones:strings(raw.supported_time_zones,['Asia/Jerusalem','UTC']),weekly_schedule:schedule(raw.weekly_schedule),
 ...
 auto_resume_hours:number(raw.auto_resume_hours,0),deferred_max_age_hours:number(raw.deferred_max_age_hours,12),context_message_count:number(raw.context_message_count,10),context_retention_hours:number(raw.context_retention_hours,48),
 ...
 campaign_routes:routes(raw.campaign_routes,'keyword'),source_routes:routes(raw.source_routes,'source'),templates:object(raw.templates),owner_phone:typeof raw.owner_phone==='string'?raw.owner_phone:'',paired:raw.paired===true,exceptions:Array.isArray(raw.exceptions)?raw.exceptions:[],
};}
