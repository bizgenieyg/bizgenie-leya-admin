...
export function normalizeTenantSettings(value:unknown){const raw=object(value);return{
 ...
 auto_replies_paused:raw.auto_replies_paused===true,translate_owner_answer:raw.translate_owner_answer===true,polish_owner_answer:raw.polish_owner_answer!==false,client_discovery_questions:strings(raw.client_discovery_questions,[]).slice(0,10),escalation_remind_minutes:number(raw.escalation_remind_minutes,120),escalation_close_minutes:number(raw.escalation_close_minutes,1440),
 auto_resume_hours:number(raw.auto_resume_hours,4),deferred_max_age_hours:number(raw.deferred_max_age_hours,12),context_message_count:number(raw.context_message_count,10),context_retention_hours:number(raw.context_retention_hours,48),message_retention_days:number(raw.message_retention_days,30),
 enabled_agents:strings(raw.enabled_agents,['SALE','SUPPORT']),intent_confidence_threshold:number(raw.intent_confidence_threshold,.75),route_stickiness_hours:number(raw.route_stickiness_hours,24),reception_max_messages:number(raw.reception_max_messages,0),simulator_hourly_limit:number(raw.simulator_hourly_limit,30),simulator_daily_limit:number(raw.simulator_daily_limit,100),
