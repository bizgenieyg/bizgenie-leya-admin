...
import {isMutation,isSameOrigin} from '@/lib/backend/same-origin';
export async function tenantBackend(request:Request,roles:readonly TenantRole[]=['owner','admin']){
 if(isMutation(request)&&!isSameOrigin(request))return{error:Response.json({code:'settingsRestricted'},{status:403})} as const;
 ...
 const call=async(path:string,init:RequestInit={},timeoutMs=15000)=>{const url=new URL(path,apiUrl);url.searchParams.set('tenantId',membership.data[0].tenant_id);return fetch(url,{...init,headers:{Authorization:`Bearer ${adminApiKey}`,'Content-Type':'application/json',...(init.headers??{})},cache:'no-store',redirect:'error',signal:AbortSignal.timeout(timeoutMs)});};
