...
export async function tenantBackend(request:Request,roles:readonly TenantRole[]=['owner','admin']){
 ...
 if(!process.env.LEIA_API_URL||!process.env.LEIA_ADMIN_API_KEY)return{error:Response.json({error:'Сервис временно недоступен.'},{status:503})} as const;
 const call=async(path:string,init:RequestInit={})=>{const url=new URL(path,process.env.LEIA_API_URL);url.searchParams.set('tenantId',membership.data[0].tenant_id);return fetch(url,{...init,headers:{Authorization:`Bearer ${process.env.LEIA_ADMIN_API_KEY}`,'Content-Type':'application/json',...(init.headers??{})},cache:'no-store',redirect:'error',signal:AbortSignal.timeout(15000)});};
