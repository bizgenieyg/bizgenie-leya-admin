...
export default function GroupDirectory(){
  ...
  return <ul className="group-list">{groups.map(group=><li key={group.id} className="group-card"><span className="group-avatar" aria-hidden="true">{initials(group.name)}</span><span className="group-copy"><strong>{group.name}</strong><small>{t('groupMembers',{count:format.number(group.participantsCount)})}</small></span>{group.lastActivityAt?<time dateTime={group.lastActivityAt}>{format.time(group.lastActivityAt)}</time>:null}</li>)}</ul>;
}
