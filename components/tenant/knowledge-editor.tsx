...
export default function KnowledgeEditor({ onboarding = false }: { onboarding?: boolean }) {
  ...
  return (
    <>
      {onboarding ? <p className="mb-5 text-sm text-gray-500">Добавьте вопросы и ответы или завершите настройку без FAQ.</p> : null}
      {loading ? <p role="status">Загрузка...</p> : tenantId && !items.length ? <p className="mb-5 text-sm text-gray-500">{onboarding ? 'Пока нет вопросов.' : 'Пока нет ни одного вопроса'}</p> : null}
      {!loading && canEdit && !items.length && !onboarding ? <button type="button" className="mb-4 text-sm text-blue-600" onClick={() => questionInput.current?.focus()}>Добавить вопрос</button> : null}
      {!loading && tenantId && !canEdit ? <p className="mb-4 text-sm text-gray-500">Вопросы доступны только для просмотра.</p> : null}
        ...
        {items.map((item) => (
          <li key={item.id} className="rounded-xl border border-gray-200 p-4">
            ...
            <div className="mt-3 flex gap-4 text-sm">
              <button type="button" disabled={busy || !canEdit} className="text-blue-600 disabled:opacity-50" onClick={() => { setEditingId(item.id); setQuestion(item.question ?? ''); setAnswer(item.answer); questionInput.current?.focus(); }}>Редактировать</button>
              <button type="button" disabled={busy || !canEdit} className="text-red-600 disabled:opacity-50" onClick={() => void removeItem(item.id)}>Удалить</button>
            </div>
          </li>
        ))}
      </ul>
