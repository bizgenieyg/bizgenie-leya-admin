...
const compiled = ts.transpileModule(readFileSync('app/onboarding/step-3/page.tsx', 'utf8'), {
  compilerOptions: { module: ts.ModuleKind.CommonJS, jsx: ts.JsxEmit.ReactJSX, target: ts.ScriptTarget.ES2020 },
}).outputText;
function harness() {
  let time = 1000000, index = 0, serial = 0, dirty = true, tree, status = 'SCAN_QR_CODE';
  ...
  vm.runInNewContext(compiled, {
    exports, AbortController, Date: { now: () => time },
    ...
    require(name) {
      ...
      if (name === '@/lib/waha/connection') return {
        isConnected: status => status === 'WORKING', isFailed: status => ['FAILED', 'STOPPED'].includes(status), backendUnavailable: 'Бэкенд недоступен.',
        beginConnection: async signal => { calls.push({ kind: 'start', signal }); return status; },
        requestStatus: async (_path, signal) => { calls.push({ kind: 'poll', signal }); return status; },
      };
      return {};
    },
  });
  function render() {
    while (dirty) { dirty = false; index = 0; tree = exports.default(); while (effects.length) effects.shift()(); }
    return tree;
  }
  ...
  return { render, calls, timers, advance, setStatus: next => { status = next; },
    start: async () => { nodes(tree).find(node => node.type === 'button' && node.props.children === 'Подключить WhatsApp').props.onClick(); render(); await settle(); },
...
test('timeout aborts after three minutes and removes QR', async () => {
  ...
  assert.ok(nodes(app.render()).some(node => typeof node.props.children === 'string' && node.props.children.startsWith('Истекло время ожидания.')));
});
