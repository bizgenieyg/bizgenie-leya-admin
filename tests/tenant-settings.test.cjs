...
vm.runInNewContext(compile('app/api/tenant-settings/route.ts'),{exports,require:n=>n==='server-only'?{}:n.includes('leya-env')?{readLeyaBackendEnv:()=>({apiUrl:'https://backend.invalid',adminApiKey:'secret'})}:n.includes('roles')?roles:n.includes('normalize')?normalizer:n.includes('fields')?fields:{createClient:()=>db},URL,Response,AbortSignal,console:{error(){}},process:{env:{}},fetch:async(url,options)=>{calls.push({url:String(url),options});return Response.json({saved:true});}});return{...exports,calls};}
